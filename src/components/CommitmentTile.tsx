...
export default function CommitmentTile({
  ...
  /* target = where the pointer is; cur = where the light is. 0.14 per frame
     settles in roughly 150ms, which trails the cursor enough to read as motion
  ...
  /* The loop parks itself once the light has arrived, so an idle tile costs
     nothing; the next pointermove restarts it. */
  ...
  const spot = (e: React.PointerEvent<HTMLElement>) => {
    ...
    target.current = [e.clientX - r.left, e.clientY - r.top];
    /* First contact starts where the pointer is, or the light visibly flies in
       from the tile's centre. */
